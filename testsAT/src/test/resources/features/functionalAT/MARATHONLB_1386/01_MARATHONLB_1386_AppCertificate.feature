...
#    And I run 'sudo echo "nginx-qa | "DNS:nginx-qa.labs.stratio.com" | client-server | userland/certificates/nginx-qa" >> /stratio_volume/certs_custom_app_marathonlb.list' in the ssh connection
#    And I run 'grep -Po '"internalDomain":\s"(\d*?,|.*?[^\\]")' /stratio_volume/descriptor.json | awk -F":" '{print $2}' | sed -e 's/^\s"//' -e 's/"$//'' in the ssh connection and save the value in environment variable 'internal_Domain'
#    And I save '!{internal_Domain}' in variable 'internalDomain'
#    And I run 'grep -Po '"consulDatacenter":\s"(\d*?,|.*?[^\\]")' /stratio_volume/descriptor.json | awk -F":" '{print $2}' | sed -e 's/^\s"//' -e 's/"$//'' in the ssh connection and save the value in environment variable 'consul_Datacenter'
#    And I save '!{consul_Datacenter}' in variable 'consulDatacenter'
#    And I run 'docker ps | grep eos-installer | awk '{print $1}'' in the ssh connection and save the value in environment variable 'containerId'
#    And I run 'docker exec -t !{containerId} cd /stratio/paas-secret-utils/ && ./gencerts -l /stratio_volume/certs_custom_app_marathonlb.list -w -v vault.service.!{internalDomain} -o 8200 -t !{vaultToken} -d !{internalDomain} -c !{consulDatacenter} ' in the ssh connection
